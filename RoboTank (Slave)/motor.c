 ...
 * this step is unnecessary). In order to return to the top menu select and all
    ...
    while(1) {
        ...
        while(key != 0x1F) { // leave menu selection by pressing all 5 keys
            if(menuSel == manual) { // manual mode
                ...
                // check for new obstacles using the instant distances
                bowObs = getConvertedDistance(0) <= 150; 
                sternObs = getConvertedDistance(3) <= 150;
                
                // setup the buzzer frequency based on the average rear distance
                uint32_t ulPeriod = SysCtlClockGet() / getConvertedDistance(5);
            ...
            }else { // autonomous wall following
          
              // Start going forward
              motorControl(standby, standby, highSpeed, highSpeed);
              
              // Enable PWMs
              PWMOutputState(PWM_BASE, PWM_OUT_3_BIT, 1);
              PWMOutputState(PWM_BASE, PWM_OUT_2_BIT, 1);
              
              while(key != up) { // up escapes
                
                // if there is an obstacle in front
                if(getConvertedDistance(0) <= 150) { 
                  //turn right
                  motorControl(reverse, forward, highSpeed, highSpeed); 
                  // for 5 seconds
                  vTaskDelay(500);
                  
                // if the wall on the left is more than 70 cm away
                } else if(getConvertedDistance(1) >= 700) {
                  // turn left
                  motorControl(forward, reverse, highSpeed, highSpeed); 
                  // for 5 seconds
                  vTaskDelay(500);
                  
                // if the wall on the left is less than 10 cm away
                } else if(getConvertedDistance(1) <= 100) {
                  // turn right
                  motorControl(reverse, forward, highSpeed, highSpeed);
                  // for 1 second
                  vTaskDelay(100);
                
                // in all other cases just go straight
                } else
                  motorControl(forward, forward, highSpeed, highSpeed);
                  
                // in the rare case that there has been nothing to the left for 
                // an entire second
                if(getConvertedDistance(5) <= 500)
                  // go straight
                  motorControl(forward, forward, highSpeed, highSpeed); 
                
                xQueueReceive(xKeyQueue, &key, 20); // get key
                
                // wait some time for more time to go in all cases
                vTaskDelay(20); 
